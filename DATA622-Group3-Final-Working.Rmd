...
Data preprocessing often begins by addressing missingness. EDA found that, regardless of feature, this data set contains zero missing values. Completeness is fortunate in that it renders imputation and the associated assumptions unnecessary. 
...
Next comes assessing the features for possible power transformation. Below are skewness statistics for each feature, with negative values reflecting left skewness and positive values reflecting right skewness. Larger values are associated with greater levels of skewness.
...
```{r correlations4}
corr <- cor(df[,-22], method = "spearman")
corrplot::corrplot(corr)
findCorrelation(corr, names = TRUE, exact = TRUE)
```

#### Outliers

Outliers present in the data set may be a concern depending upon the modeling approach. This analysis uses multinomial regression, which is relatively sensitive to outliers, and SVM and tree-based models, which are less so. The EDA boxplots revealed outliers--defined as samples with values greater than the upper/lower bounds of the inter-quartile range (IQR) plus/minus 1.5 times the IQR--across features by class of `fetal_health`. One possible treatment of these outliers is the spatial sign transformation, which projects all samples onto a unit sphere such that they are all equidistant from the center of that sphere.

#### Train/Test Split

Lastly, the highly correlated features are removed, and the data set is split 70/30 into a training set and a test set. The latter will be held out for validation of the final models.
...
The output shows that the model accuracy for test set is 89.09%.
...
Predict another model with Cost value of 0.25.
...
The output shows that the improved model accuracy for test set is 89.71%.

#### SVM Conclusion
SVlinear is chosen in this project because of the linear values of the dataset. Using Cost value of 0.25 to make a more improved SVM model, the improved model predicts accurately with 89.71% for the 3 classes of fetal health: 'Normal', 'Suspect', and 'Pathological'.
...
(gbm1 <- train(x = train_set[,-19],
                           ...
                           method = "gbm",
                           preProcess = c('center', 'scale'),
                           trControl = trctrl, #trainControl(method = "repeatedcv", number = 10, repeats = 3, sampling = "smote"),
                           verbose = FALSE))
gbm1_pred <- predict(gbm1, newdata = test_set)
postResample(pred = gbm1_pred, obs = test_set$fetal_health)
```
```{r gbm1_matrix}
confusionMatrix(table(gbm1_pred, test_set$fetal_health))
```
```{r gbm_grid}
...
postResample(pred = gbm2_pred, obs = test_set$fetal_health)
```
```{r gbm2_matrix}
confusionMatrix(table(gbm2_pred, test_set$fetal_health))
```
